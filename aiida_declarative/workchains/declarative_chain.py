...
def dict2datanode(dat, typ, dynamic=False):
    """Convert a dict read from json or yaml into an aiida datanode of 'typ'"""
    # Resolve recursively
    if dynamic:
        out = dict()
        for k in dat:
...
def set_dot2index(d, key, val):
    """Allows for overwriting certain values in a dictionary.
    e.g.
    ```yaml
    inputs:
        parameters:
...
class DeclarativeChain(WorkChain):
    """Meta workchain to self-assemble and run a yaml or json specified workflow."""
    @classmethod
    def define(cls, spec):
        super().define(spec)
    ...
    def setup(self):
        self.ctx._current_id = 0
        self.ctx._results = dict()
    ...
    def not_finished(self):
        return self.ctx._current_id < len(self.ctx._steps)
    ...
    def submit_next(self):
        ...
        if isinstance(n, Node):
            ...
            if is_process_function(cjob):
                return ToContext(_current=run_get_node(cjob, **inputs)[1])
            else:
                return ToContext(_current=self.submit(cjob, **inputs))

    def next_step(self):

        id = self.ctx._current_id
        step = self.ctx._steps[id]

        if "if" in step and not self.eval_template(step["if"]):
            self.ctx._current_id += 1
            return self.next_step()

        elif "while" in step:
            if self.eval_template(step["while"]):
                if not self.ctx._in_while:
                    # Enter the while loop
                    self.ctx._in_while = True
                    self.ctx._while_first_id = len(self.ctx._steps)
                    self.ctx._while_entry_id = id
                    self.ctx._steps += step["steps"]
                    self.ctx._current_id = self.ctx._while_first_id
                else:
                    # Reroll the while loop
                    self.ctx._current_id = self.ctx._while_first_id

            elif self.ctx._in_while:
                # Cleanup while loop
                self.ctx._in_while = False
                self.ctx._steps = self.ctx._steps[0 : -len(step["steps"])]
                self.ctx._current_id += 1
    ...
    def resolve_inputs(self, inputs, spec_inputs):
        """Recursively resolves the DataNodes to be used as inputs in the step"""
        out = dict()
        for k in inputs:
    ...
    def process_current(self):
        """Executes the postprocessing part of a step if specified"""
        step = self.ctx._steps[self.ctx._current_id]

        if not self.ctx._current.is_finished_ok:
            self.report(
                f"A subprocess failed with exit status {self.ctx._current.exit_status}: {self.ctx._current.exit_message}"
            )
            if "error" in step:
        ...
        if self.ctx._in_while and self.ctx._current_id == len(self.ctx._steps):
            self.ctx._current_id = self.ctx._while_entry_id

    # Jinja evaluation
    def eval_template(self, s):
        if isinstance(s, str) and '{{' in s and '}}' in s:
            return self.env.from_string(s).render(ctx=self.ctx, outputs = self.ctx._current.outputs)
        return s
    ...
    def to_ctx(self, value, key):
        if key in self.ctx:
            self.report(
                f"to_ctx: key '{key}' already exists in the workchain context, overwriting its value."
            )
        self.ctx[key] = value
        return value
    ...
    def to_results(self, value, key):
        if key in self.ctx._results:
            self.report(
                f"to_results: key '{key}' already exists in the workchain results, overwriting its value."
            )
        self.ctx._results[key] = value
        return value

    def finalize(self):
        """Attach gathered results to the results output node of the workchain"""
        self.out("results", self.ctx._results)
